...
\begin{posterboxenv}[title=Eine Box im Stil FZJ-colored, FZJ-colored]{name=colored,column=3, row=3, span=2}
	...
	Einstellung über globale Klassenoption \code{style=colored} oder lokaler Stil \code{FZJ-colored} 

\end{posterboxenv}

\begin{posterboxenv}[FZJ-colored]{name=colored-notitle,column=3,row=4, rowspan=2}
	\includegraphics[width=\linewidth]{example-image}
	\captionof{figure}{Ein Beispielbild, in einer Box ohne Titel. In diesem Fall sind der Stil {FZJ} und FZJ-colored identisch}
\end{posterboxenv}

\begin{posterboxenv}[FZJ-boxed]{name=boxed-notitle,column=4, below=colored}
	Ein Beispiel für die den Stil \code{boxed} ohne Titel.
\end{posterboxenv}\pck{tc


\begin{posterboxenv}[title=Box mit Link,FZJ-boxed]{name=relative,column=4, above=row6}
...
\begin{posterboxenv}[title=relative Positionierung,FZJ-boxed]{name=relative,column=4, between=boxed-notitle and relative}
...
Die Klasse \cls{fzjsciposter} unterstützt die Papierformate A0, A1, A2 und A3. Der Wert wird über die Klassenoption \code{paper} ausgewählt:
